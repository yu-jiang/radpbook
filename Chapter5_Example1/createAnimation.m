...
LocalCreateAnimation(t,pmgr.angle10,delta + pmgr.angle10, ...
    pmgr.angle20,delta1 + pmgr.angle20,true,1*(t>=2)+1*(t>=3), pmgr)
end
end


function LocalCreateAnimation(t,delta,delta0,delta1,delta10,ifct,ifadp, pmgr)
B12 = pmgr.B12; 
B11 = pmgr.B11;
Ef1 = pmgr.Ef1;
Ef2 = pmgr.Ef2;
...
figure(3)
set(figure(3),'color',[255,255,255]/255)
%delta=1;
r1=0.15;
r2=0.1;
...
if ifct==1
    ...
    %draw the connection
    line([.13,0.13]+0.1,[y0,y0+0.16],'color',[0,.5,0],'linewidth',2)
    line([.13,0.47]+0.1,[y0+0.16,y0+0.16],'color',[0,.5,0],'linewidth',2)
    line([.47,0.47]+0.1,[y0+0.16,y0],'color',[0,.5,0],'linewidth',2)
    line([.57,x1-r2],[y0,y0],'color',[0,.5,0],'linewidth',2)
    line([.62,0.72],[y0,y0],'color',[0,.5,0],'linewidth',4)
else
    patch([.73 .73 .87 .87]+0.05,[0.2 .25 .25 .2],[0,0,0])
    text(0.6,0.45,'Disonnected','fontsize',10,'color',[0,0,0])
    %draw the connection
    line([.13,0.13]+0.1,[y0,y0+0.16],'color',[0,.5,0],'linewidth',2,'linestyle',':','color','c')
    line([.13,0.47]+0.1,[y0+0.16,y0+0.16],'color',[0,.5,0],'linewidth',2,'linestyle',':','color','c')
    line([.47,0.47]+0.1,[y0+0.16,y0],'color',[0,.5,0],'linewidth',2,'linestyle',':','color','c')
    line([.57,x1-r2],[y0,y0],'color',[0,.5,0],'linewidth',2,'linestyle',':','color','c')
    %line([.62,0.72],[y0,y0],'color',[0,.5,0],'linewidth',4,'linestyle',':')
end
%===================================================
